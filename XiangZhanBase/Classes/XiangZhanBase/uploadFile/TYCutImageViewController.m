...
- (void)createCutImage {
    UIView*view=[[UIView alloc]initWithFrame:CGRectMake(0,HEIGHT-(kDevice_Is_iPhoneX ? 94 : 60) , WIDTH, 60)];
    ...
    for (int i=0; i<2; i++) {
        ...
        if (i==0) {
            ...
            button.frame=CGRectMake(0, HEIGHT-(kDevice_Is_iPhoneX ? 94 : 60), 60, 60);
        }else{
            [button setTitle:@"选取" forState:UIControlStateNormal];
            button.frame=CGRectMake(WIDTH-60,  HEIGHT-(kDevice_Is_iPhoneX ? 94 : 60), 60, 60);
        }
    }
}
