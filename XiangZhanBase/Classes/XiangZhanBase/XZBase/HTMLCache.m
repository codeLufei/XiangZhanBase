...
- (NSString *)objectForKey:(NSString *)key {
    ...
    if (html.length <= 0 && [Reachability reachabilityForInternetConnection].currentReachabilityStatus == NotReachable) {
...
// NSString *cacheTime = [jsonDic objectForKey:@"expires"];
//    //使用期间缓存，每次重新打开app后从服务器请求
//    if ([cacheMethod isEqualToString:@"private"]) {
//        [[HTMLCache sharedCache] runtimeCacheString:htmlStr forKey:urlStr];
//    }
//    //按expires的过期时间，过期后从服务器请求
//    else if ([cacheMethod isEqualToString:@"max-age"]) {
//        [[HTMLCache sharedCache] setString:htmlStr forKey:urlStr withTimeoutInterval:cacheTime.doubleValue];
//    }
//    //永久缓存
//    else if ([cacheMethod isEqualToString:@"must-revalidate"]) {
