...
@interface CFJWebViewBaseController ()<UIWebViewDelegate,UIScrollViewDelegate,UIGestureRecognizerDelegate,UINavigationControllerDelegate>
{
...
- (void)loadHtml {
    if (self.htmlStr) {
        ...
        if ([[NSUserDefaults standardUserDefaults]  boolForKey:@"isCopy"]) {
            WEAK_SELF;
            dispatch_async(dispatch_get_global_queue(0, 0), ^(void) {
                STRONG_SELF;
                NSString * headerHtml = [HTMLCache sharedCache].pageHtml;
            ...
            dispatch_async(dispatch_get_global_queue(0, 0), ^(void) {
                STRONG_SELF;
                NSString * headerHtml = [HTMLCache sharedCache].noPageHtml;
                NSString *allHtmlStr = [headerHtml stringByReplacingOccurrencesOfString:@"<!--body-->" withString:self.htmlStr];
                dispatch_async(dispatch_get_main_queue(), ^(void) {
                    STRONG_SELF;
...
- (void)loadAppHtml {
    if (self.htmlStr) {
        ...
        headerHtml = [HTMLCache sharedCache].appHtml;
        NSString *allHtmlStr = [headerHtml stringByReplacingOccurrencesOfString:@"<!--body-->" withString:self.htmlStr];
        [self.webView loadHTMLString:allHtmlStr baseURL:[HTMLCache sharedCache].htmlBaseUrl];
        //注意，一个webview bridge只能有一次，否则失败
        if (!self.bridge) {
...
- (void)downloadHtmlBodyWithUrl:(NSString *)urlStr needLoadPage:(BOOL)loadPage complete:(DownloadBodyFinish)callBack {
    ...
    if (self.htmlStr) {
        ...
        [manager GET:urlStr parameters:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
            ...
            NSLog(@"请求body错误::::%@",error.description);
        }];
        return;
    }
