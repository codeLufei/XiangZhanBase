...
+ (NSString *)saveAssetToTmp:(ALAsset *)asset WithQuality:(CGFloat)aQuality {
    if (aQuality < 0) {
        aQuality = 0;
    }
    if (aQuality > 1.0f) {
    ...
    NSString *path = [NSString stringWithFormat:@"%@%ld%@",[BaseFileManager appTmpPath],(long)[NSDate date].timeIntervalSince1970,[[asset defaultRepresentation] filename]];
    UIImage *fullResolutionImage = [UIImage imageWithCGImage:[asset.defaultRepresentation fullResolutionImage]];
    NSData* imageData = UIImageJPEGRepresentation(fullResolutionImage, aQuality);
...
+ (UIImage *)watermarkImage:(UIImage *)img withData:(id)setDic
{
    ...
    paragraphStyle.lineBreakMode = UILineBreakModeCharacterWrap;
